...
import { ZERO, INFINITY, ONE, TradeRouter, PoolService, Router, BigNumber } from '@galacticcouncil/sdk';
...
export async function getBsxSwapExtrinsic(startAssetSymbol: string, destAssetSymbol: string, assetInAmount: number, assetOutAmount: number, swapInstructions: any[]) {
    ...
    let formattedInputAmount = new BigNumber(assetInAmount).times(new BigNumber(10).pow(assetIn.decimals))
    let fnInputAmount = new FixedPointNumber(assetInAmount.toString(), assetIn.decimals).toChainData()
    let bestBuy = await router.getBestSell(assetIn.id, assetOut.id, formattedInputAmount)
