...
function getAssetFromSymbol(symbol: string){
    const asset = assetRegistry.find((asset: any) => asset.tokenData.chain == "2023" && asset.tokenData.symbol.toLowerCase() == symbol.toLowerCase())
    if(!asset){
...
function getAssetFromLocalId(localId: string){
    const asset = assetRegistry.find((asset: any) => asset.tokenData.chain == 2023 && asset.tokenData.localId.toLowerCase() == localId.toLowerCase())
    if(!asset){
...
//     if(!wallet.provider) throw new Error("Error MOVR Wallet provider")
...
export async function getMovrSwapTx(swapInstructions: SwapInstruction[], chopsticks: boolean): Promise<BatchSwapParams>{
    ...
    if(chopsticks){
        ...
        // if (!live_wallet_3) throw new Error('LIVE_WALLET_3_PK is not defined');
...
export async function testXcTokensMoonriver(tokenPath: string[], inputAmountNumber: number, slippage = 50){
    ...
    let liveWallet = new ethers.Wallet(live_wallet_3, liveProvider)
