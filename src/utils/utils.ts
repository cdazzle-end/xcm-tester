...
import { arb_wallet_kusama, dotTargetNode, ksmTargetNode, live_wallet_3 } from '../config/txConsts.ts';
import { ArbExecutionResult, AsyncFileData, ExtrinsicSetResultDynamic, IndexObject, InstructionType, JsonPathNode, LastNode, MyAssetRegistryObject, PathData, PathType, Relay, SingleSwapResultData, SingleTransferResultData, SwapInstruction, SwapTxStats, TransferInstruction, TransferTxStats, TxDetails } from './../types/types.ts';
...
export function getAssetRegistry(relay: Relay){
    const assetRegistryPath = relay === 'kusama' ? '../../../polkadot_assets/assets/asset_registry/allAssetsKusamaCollected.json' : '../../../polkadot_assets/assets/asset_registry/allAssetsPolkadotCollected.json'
    ...
    const assetRegistry: MyAssetRegistryObject[] = JSON.parse(fs.readFileSync(path.join(__dirname, assetRegistryPath), 'utf8'));
